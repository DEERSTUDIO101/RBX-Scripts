...
local iyassets = {
	...
	["infiniteyield/assets/logo.png"] = "rbxassetid://122985738548503",
...
-- Rounded Corner für modernen Look
...
HolderStroke.Color = Color3.fromRGB(60, 60, 65)
...
CMDsF.ScrollBarThickness = 5 -- Dünnerer Scrollbar
...
cmdListLayout.Padding = UDim.new(0, 3) -- Kompakteres Padding

-- Auto-Update CanvasSize für bessere Performance mit vielen Commands
cmdListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
	CMDsF.CanvasSize = UDim2.new(0, 0, 0, cmdListLayout.AbsoluteContentSize.Y + 5)
end)
...
Example.Size = UDim2.new(1, 0, 0, 18) -- Kompakter, volle Breite
Example.Visible = false
Example.Font = Enum.Font.Gotham
Example.TextSize = 11 -- Kleinerer Text
Example.Text = "Example"
Example.TextColor3 = Color3.fromRGB(200, 200, 200) -- Leicht grauer
...
function minimizeHolder()
	if StayOpen == false then
		local currentX = Holder.Position.X.Offset
		local currentY = Holder.Position.Y.Offset
		Holder:TweenPosition(UDim2.new(Holder.Position.X.Scale, currentX, 0, minimizeNum), "InOut", "Quart", 0.5, true, nil)
	end
end
...
function cmdbarHolder()
	if StayOpen == false then
		-- Zeige Command Bar
		local currentX = Holder.Position.X.Offset
		Holder:TweenPosition(UDim2.new(Holder.Position.X.Scale, currentX, 0, 10), "InOut", "Quart", 0.5, true, nil)
	end
end
...
function UpdateToViewport()
	-- Angepasst für neue Position oben zentriert
	local holderX = Holder.Position.X.Offset
	local holderY = Holder.Position.Y.Offset
	local viewportWidth = CamViewport()
	
	-- Stelle sicher, dass Holder nicht außerhalb des Bildschirms ist
	if holderX < -viewportWidth/2 - 140 then
		Holder:TweenPosition(UDim2.new(0.5, 0, Holder.Position.Y.Scale, holderY), "InOut", "Quart", 0.04, true, nil)
	elseif holderX > viewportWidth/2 + 140 then
		Holder:TweenPosition(UDim2.new(0.5, 0, Holder.Position.Y.Scale, holderY), "InOut", "Quart", 0.04, true, nil)
	end
	
	-- Notification bleibt unten rechts
	if Notification.Position.X.Offset < -viewportWidth then
		Notification:TweenPosition(UDim2.new(1, -viewportWidth + 250, Notification.Position.Y.Scale, Notification.Position.Y.Offset), "InOut", "Quart", 0.04, true, nil)
	end
end
...
-- dragMain entfernt - neue Draggable-Funktion wird verwendet
...
for i = 1, #CMDs do
	...
	newcmd.Text = "  " .. CMDs[i].NAME -- Padding für besseres Aussehen
	newcmd.Name = "CMD"
	newcmd.LayoutOrder = i -- Für bessere Sortierung
	...
	newcmd.MouseLeave:Connect(function()
		if newcmd.Visible and newcmd.TextTransparency == 0 then
			newcmd.TextColor3 = Color3.fromRGB(200, 200, 200)
		end
	end)
